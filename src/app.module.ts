    ...
    imports: [
        ...
        TypeOrmModule.forRootAsync({ // Setup database server with TypeORM Module. We use forRootAsync instead of forRoot is because our TypeORM params depends on other Module's values instead of directly providing the value (hardcoding)
            ...
            useFactory: (config: ConfigService) => ({
                ...
                host: config.getOrThrow<string>('DB_HOST'), // DB Hostname, i.e. localhost or AWS/GCP/Azure hostname address
                port: config.getOrThrow<number>('DB_PORT'), // DB Port Number
                username: config.getOrThrow<string>('DB_USERNAME'), // DB Acc Username
                password: config.getOrThrow<string>('DB_PASSWORD'), // DB Password
                database: config.getOrThrow<string>('DB_NAME'), // DB Name
